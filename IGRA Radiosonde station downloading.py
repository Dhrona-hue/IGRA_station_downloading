...
stns_ftr1 = stns_ftr1.loc[(stns_ftr1['total']>=40000)]
...
for m in range(0,123):
    ...
    try:
        ...
        if "No data available" in str(e):
            # No data available for this station, continue to the next station
            continue
        else:
            # Handle other ValueError exceptions as needed
            raise e
    
    # Check if the data DataFrame is empty
    if df is None or df.empty:
        # No data found for this station, continue to the next station
        continue
    ...
    Matrixx.to_excel(station + '_' + stn_name.replace("/", " ") + '_' + str(stn_lat).replace('.', 'dot') + '.xlsx')
    
    # Print the loop number
    print(f"Processed loop number {m}")
